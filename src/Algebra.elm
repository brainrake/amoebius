...
show_transform t =
    if t == ident then
        "ident"
    else if t == flip_x then
        "flip_x"
    else if t == flip_y then
        "flip_y"
    else if t == rot_r then
        "rot_r"
    else if t == rot_l then
        "rot_l"
    else
        toString t
...
make_lattice right down left up =
    if
        True
            ...
            && (left *** down == down *** left)
    then
        Just (Lattice right down left up)
    else
        Nothing
...
gen_lattice =
    let
        get i xs =
            (xs !! i ? ident)
    in
        Random.list 4 gen_transform
            |> Random.map (\xs -> Lattice (get 0 xs) (get 1 xs) (get 2 xs) (get 3 xs))


moebius : Lattice
moebius =
    Lattice ident flip_x ident flip_x


vortex : Lattice
vortex =
    Lattice rot_r rot_l rot_r rot_l
